...
%TRANSMAP M-file for TRANSMAP.fig
%      TRANSMAP, by itself, creates a new TRANSMAP or raises the existing
%      singleton*.
%
%      H = TRANSMAP returns the handle to a new TRANSMAP or the handle to
%      the existing singleton*.
%
%      TRANSMAP('Property','Value',...) creates a new TRANSMAP using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to TRANSMAP_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      TRANSMAP('CALLBACK') and TRANSMAP('CALLBACK',hObject,...) call the
%      local function named CALLBACK in TRANSMAP.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help TRANSMAP

% Last Modified by GUIDE v2.5 26-Nov-2015 02:11:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TRANSMAP_OpeningFcn, ...
                   'gui_OutputFcn',  @TRANSMAP_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before TRANSMAP is made visible.
function TRANSMAP_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for TRANSMAP
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes TRANSMAP wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TRANSMAP_OutputFcn(hObject, eventdata, handles)
...
line([PA(1,1),PX],[PA(2,1),PY],[PA(3,1),PZ],'Color','k');
hold off
axis equal
